...
function setupSearchToggle(){
	const saved =
		localStorage.getItem(SEARCH_KEY) === "true";
	...
	toggleSearch?.addEventListener("click", () => {
		...
		const hidden =
			document.body.classList.contains("hide-search");

		toggleSearch.textContent =
			hidden ? "Show Search" : "Hide Search";

		localStorage.setItem(SEARCH_KEY, hidden);
	});
}
...
function setupGoToggle(){
	...
	const saved =
		localStorage.getItem(GO_KEY) === "true";

	if(saved){
		document.body.classList.add("hide-go");
	...
	toggleGo?.addEventListener("click", () => {
		...
		const hidden =
			document.body.classList.contains("hide-go");

		toggleGo.textContent =
			hidden ? "Show Go" : "Hide Go";

		localStorage.setItem(GO_KEY, hidden);
	});
}
// ==============================
// DEFAULT LOADS
