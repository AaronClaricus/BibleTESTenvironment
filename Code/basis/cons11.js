...
const UIState = {
    ...
    applySideEffects(key, value) {
        switch (key) {
            ...
            case "layoutMode":
                // IMPORTANT: ensure state is updated BEFORE DOM
                APP.state.layoutMode = value;
                applyLayoutMode();
                break;
...
const UIService = {
    getHighlightScheme() {
        ...
        return highlightSchemes[selected] || {
            bg: "#000",
            text: "#fff"
        };
    }
...
const TemplateService = {
    async ensure() {
        if (APP.state.templateHTML) return;

        const response = await fetch("./Code/template.html");
        APP.state.templateHTML = await response.text();
    },
...
UIService.getHighlightScheme(selected);
